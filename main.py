...
class Node:
  def __init__(self):
    ...
    self.children = []
    #weight of connection to children
    self.weight = []
  ...
  def make_children(self, current_layer_number, node_per_layer_map):
    ...
    #self.children are all node children for this level
  ...
  def print_nodes(self, current_layer_number, node_per_layer_map):
    ...
    for i in range(len(self.children)):
      ...
      self.children[i].print_nodes(current_layer_number + 1, node_per_layer_map)
    return
...
new_node.print_nodes(0, NODE_COUNT_PER_LAYER)
